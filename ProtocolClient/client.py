...
class MessagingChannelHandler():
    def __init__(self, address: Tuple[str, int], timeout: float = 0.1, sleepingTime: int = 1,
                 side: str = PacketType.ALL, isDebug: bool = False) -> None:
    ...
    def _MainThread(self) -> None:
        while self._isWorking:
            ...
            for data in self.fa_responses:
                ...
                if isAuthed:
                    self.sendPacket(PacketType.get_SITESIDE_2FA_RESPONSE(player, PacketType.SUCCESS, ""))
                else:
                    self.sendPacket(PacketType.get_SITESIDE_2FA_RESPONSE(player, PacketType.DENIED, "UNAFTORIZED"))
